...
export const gitTools: ToolHandler[] = [
  ...
  },
  {
    ...
    handler: async (args, { services }) => {
      ...
      return commits.map(c =>
        `${c.hash.substring(0, 8)} | ${c.date} | ${c.author} | ${c.message}`
      ).join('\n');
    }
  },
  {
    ...
    handler: async (args, { services }) => {
      const branches = await services.git.branch(
        args.action as 'list' | 'create' | 'delete' | 'current',
  ...
  },
  {
    ...
    handler: async (args, { services }) => {
      const result = await services.git.checkout(
        args.target as string,
        args.create_branch as boolean | undefined
      );
  ...
  },
  {
    ...
    handler: async (args, { services }) => {
      await services.git.add(args.path as string);
  ...
  },
  {
    ...
    handler: async (args, { services }) => {
      const result = await services.git.stash(
        args.action as 'push' | 'pop' | 'list' | 'drop',
  ...
  },
  {
    ...
    handler: async (args, { services }) => {
      const entries = await services.git.blame(args.path as string);
      return entries.map(e =>
        `${String(e.line).padStart(4)} | ${e.hash.substring(0, 8)} | ${e.author.padEnd(20)} | ${e.content}`
      ).join('\n');
    }
  }
];
