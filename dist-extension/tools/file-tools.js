...
exports.fileTools = [
    {
        ...
        handler: async (args, { services }) => {
            const content = await services.fileSystem.readFile(args.path);
            return content;
        }
    },
    {
        ...
        handler: async (args, { services }) => {
            ...
            await services.fileSystem.writeFile(args.path, content);
    ...
    },
    {
        ...
        handler: async (args, { services }) => {
            const { replacedCount, totalCount } = await services.fileSystem.editFile(args.path, args.old_text, args.new_text, {
    ...
    },
    {
        ...
        handler: async (args, { services }) => {
            const entries = await services.fileSystem.listDirectory(args.path || '.', {
    ...
    },
    {
        ...
        handler: async (args, { services }) => {
            const info = await services.fileSystem.getFileInfo(args.path);
            return [
                `Name: ${info.name}`,
                `Path: ${info.path}`,
                `Type: ${info.type}`,
    ...
    },
    {
        ...
        handler: async (args, { services }) => {
            const files = await services.fileSystem.findFiles(args.pattern);
            if (files.length === 0)
    ...
    },
    {
        ...
        handler: async (args, { services }) => {
            const results = await services.fileSystem.readMultipleFiles(args.paths, args.max_files || 10);
    ...
    },
    {
        ...
        handler: async (args, { services }) => {
            await services.fileSystem.deleteFile(args.path, args.recursive);
    ...
    },
    {
        ...
        handler: async (args, { services }) => {
            await services.fileSystem.moveFile(args.source, args.destination);
